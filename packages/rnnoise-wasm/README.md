...
> **Monorepo note:** vendored into the susurro monorepo as `packages/rnnoise-wasm` (provides `@jitsi/rnnoise-wasm` locally; consumed by `packages/murmuraba` via the prebuilt `dist/`). The prebuilt `dist/rnnoise.wasm` is committed, so no rebuild is needed to consume it. The `rnnoise` C-source git submodule (`https://github.com/xiph/rnnoise.git`) was dropped during consolidation — to rebuild the WASM from C source, re-add it with `git submodule add https://github.com/xiph/rnnoise.git rnnoise` before running `build.sh`.
